(function ($, undefined) {
  ...
  //
  // The selector is passed a date in ms and elements are added to the selection
  // filter if the element date matches, as determined by the id attribute
  // containing a parsable date in ms.
  ...
  // Date prototype helpers ====================== `getWeekId` returns a string in
  // the form of 'dh-YYYY-WW', where WW is the week # for the year. It is used to
  // add an id to the week divs
  ...
  // TODO: fix bug when firstDay=6/weekOneDate=1 :
  // https://github.com/moment/moment/issues/2115
  ...
  function ktkGetNextDate(currentDate, scaleStep) {
    for (var minIncrements = 1;; minIncrements++) {
      ...
      // If code reaches here, it's because current didn't really increment (invalid
      // local time) because of daylight-saving adjustments => retry adding 2, 3, 4
      // hours, and so on (until nextDate > current)
    }
  }
  ...
  $.fn.gantt = function (options) {
    ...
    // Grid management =============== Core object is responsible for navigation and
    // rendering
    var core = {
      // Return the element whose topmost point lies under the given point Normalizes
      // for old browsers (NOTE: doesn't work when element is outside viewport)
      ...
      create: function (element) {
        ...
        // Initialize data with a json object or fetch via an xhr request depending on
        // `settings.source`
      ...
      // **Setup the initial view** Here we calculate the number of rows, pages and
      // visible start and end dates once the data are ready
      ...
      leftPanel: function (element) {
        ...
        $.each(element.data, function (i, entry) {
          ...
          if (i >= element.pageNum * settings.itemsPerPage && i < (element.pageNum * settings.itemsPerPage + settings.itemsPerPage)) {
            var entries = [];
            var dataId = ('id' in entry)
              ? '" data-id="' + entry.id
              : '';
            entries.push('<div class="row name row' + i + (entry.desc
              ? ''
              : (' fn-wide ' + dataId)) + '" id="rowheader' + i + '" data-offset="' + i % settings.itemsPerPage * tools.getCellSize() + '"><span class="fn-label' + (entry.cssClass
              ? ' ' + entry.cssClass
              : '') + '">' + (entry.name || '') + '</span></div>');

            if (entry.desc) {
              entries.push('<div class="row desc row' + i + ' " id="RowdId_' + i + dataId + '"><span class="fn-label' + (entry.cssClass
                ? ' ' + entry.cssClass
                : '') + '">' + entry.desc + '</span></div>');
            }
            ganttLeftPanel.append($('<div>' + entries.join("") + '</div>').data('grow', entry));
          }
        });
      ...
      dataPanel: function (element, width) {
        ...
        dataPanel.click(function (e) {
          ...
          // Dispatch user registered function with the DateTime in ms and the id if the
          // clicked object is a row
      ...
      rightPanel: function (element, leftPanel/* <- never used? */) {
        var range = null;
        // Days of the week have a class of one of `sn` (Sunday), `sa` (Saturday), or
        // `wd` (Weekday)
        var dowClass = [
          "sn",
          "wd",
        ...
        // unused: was someone planning to allow styles to stretch to the bottom of the
        // chart? var gridDowClass = [" sn", "", "", "", "", "", " sa"];
        ...
        switch (settings.scale) {
          ...
          case "hours":
            ...
            for (i = 0, len = range.length; i < len; i++) {
              ...
              if (rfy !== year) {
                yearArr.push('<div class="row year" style="width: ' + tools.getCellSize() * scaleUnitsThisYear + 'px;"><div class="fn-label">' + year + '</div></div>');
              ...
              if (rm !== month) {
                monthArr.push('<div class="row month" style="width: ' + tools.getCellSize() * scaleUnitsThisMonth + 'px"><div class="fn-label">' + settings.months[month] + '</div></div>');
              ...
              if (rgetDay !== getDay) {
                ...
                dayArr.push('<div class="row date ' + dayClass + '" style="width: ' + tools.getCellSize() * hoursInDay + 'px;"><div class="fn-label">' + day.getDate() + '</div></div>');
                dowArr.push('<div class="row day ' + dayClass + '" style="width: ' + tools.getCellSize() * hoursInDay + 'px;"><div class="fn-label">' + settings.dow[getDay] + '</div></div>');
              ...
              horArr.push('<div class="row day ' + dayClass + '" id="dh-' + rday.getTime() + '" data-offset="' + i * tools.getCellSize() + '" data-repdate="' + rday.getRepDate(settings.scale) + '"><div class="fn-label">' + rday.getHours() + '</div></div>');
            }
            ...
            dowArr.push('<div class="row day ' + dayClass + '" style="width: ' + tools.getCellSize() * hoursInDay + 'px;"><div class="fn-label">' + settings.dow[day.getDay()] + '</div></div>');

            dataPanel = core.dataPanel(element, dataPanelWidth);

            // Append panel elements
            dataPanel.append($row.clone().html(yearArr.join("")), $row.clone().html(monthArr.join("")), $row.clone().html(dayArr.join("")), $row.clone().html(dowArr.join("")), $row.clone().html(horArr.join("")));
            break;
          ...
          case "weeks":
            ...
            for (i = 0, len = range.length; i < len; i++) {
              ...
              if (week > (week = rday.getWeekOfYear())) {
                ...
                diff -= !rday.getMonth()
                  ...
                  : 31;
                diff /= 7;
                yearArr.push('<div class="row year" style="width: ' + tools.getCellSize() * (scaleUnitsThisYear - diff) + 'px;"><div class="fn-label">' + year + '</div></div>');
                year++;
              ...
              if (rday.getMonth() !== month) {
                ...
                diff = rday.getDate() - 1;
                // offset one week if week starts in last month diff -= (diff <= 6) ? 0 : 7;
                diff /= 7;
                monthArr.push('<div class="row month" style="width:' + tools.getCellSize() * (scaleUnitsThisMonth - diff) + 'px;"><div class="fn-label">' + settings.months[month] + '</div></div>');
                month = rday.getMonth();
              ...
              // Fill weeks
              dayArr.push('<div class="row day wd" id="' + rday.getWeekId() + '" data-offset="' + i * tools.getCellSize() + '" data-repdate="' + rday.getRepDate(settings.scale) + '"><div class="fn-label">' + week + '</div></div>');
            }
            ...
            // Append panel elements
            dataPanel.append($row.clone().html(yearArr.join("")), $row.clone().html(monthArr.join("")), $row.clone().html(dayArr.join("")));
            break;
          ...
          case 'months':
            ...
            for (i = 0, len = range.length; i < len; i++) {
              ...
              scaleUnitsThisYear++;
              monthArr.push('<div class="row day wd" id="dh-' + tools.genId(rday) + '" data-offset="' + i * tools.getCellSize() + '" data-repdate="' + rday.getRepDate(settings.scale) + '">' + (1 + rday.getMonth()) + '</div>');
            }
          ...
          default:
            ...
            for (i = 0, len = range.length; i < len; i++) {
              ...
              dayArr.push('<div class="row date ' + dayClass + '" id="dh-' + tools.genId(rday) + '" data-offset="' + i * tools.getCellSize() + '" data-repdate="' + rday.getRepDate(settings.scale) + '"><div class="fn-label">' + rday.getDate() + '</div></div>');
              dowArr.push('<div class="row day ' + dayClass + '" id="dw-' + tools.genId(rday) + '" data-repdate="' + rday.getRepDate(settings.scale) + '"><div class="fn-label">' + settings.dow[day] + '</div></div>');
            } //for
            ...
            // Append panel elements
            dataPanel.append($row.clone().html(yearArr.join("")), $row.clone().html(monthArr.join("")), $row.clone().html(dayArr.join("")), $row.clone().html(dowArr.join("")));
        }
      ...
      navigation: function (element) {
        ...
        // Scrolling navigation is provided by setting `settings.navigate='scroll'`
        if (settings.navigate === "scroll") {
          ganttNavigate = $('<div class="navigate" />').append($('<div class="nav-slider" />').append($('<div class="nav-slider-left" />')
          /*
                          ...
                          .append($('<button type="button" class="nav-link nav-next-week"/>')
                                  ...
                                  */
            .append($('<button type="button" class="nav-link nav-zoomIn"/>').html('&#43;').click(function () {
            core.zoomInOut(element, -1);
          })).append($('<button type="button" class="nav-link nav-zoomOut"/>').html('&#45;').click(function () {
            core.zoomInOut(element, 1);
          }))));
          ...
          ganttNavigate = $('<div class="navigate" />').append($('<button type="button" class="nav-link nav-page-back"/>').html('&uarr;').click(function () {
            core.navigatePage(element, -1);
          }))
            .append($('<div class="page-number"/>').append($('<span/>').html(element.pageNum + 1 + ' / ' + element.pageCount)))
            .append($('<button type="button" class="nav-link nav-page-next"/>').html('&darr;').click(function () {
              core.navigatePage(element, 1);
            }))
            .append($('<button type="button" class="nav-link nav-begin"/>').html('&#124;&lt;').click(function () {
              core.navigateTo(element, 'begin');
            }))
            .append($('<button type="button" class="nav-link nav-prev-week"/>').html('&lt;&lt;').click(function () {
              core.navigateTo(element, tools.getCellSize() * 7);
            }))
            .append($('<button type="button" class="nav-link nav-prev-day"/>').html('&lt;').click(function () {
              core.navigateTo(element, tools.getCellSize());
            }))
            .append($('<button type="button" class="nav-link nav-now"/>').html('&#9679;').click(function () {
              core.navigateTo(element, 'now');
            }))
            .append($('<button type="button" class="nav-link nav-next-day"/>').html('&gt;').click(function () {
              core.navigateTo(element, tools.getCellSize() * -1);
            }))
            .append($('<button type="button" class="nav-link nav-next-week"/>').html('&gt;&gt;').click(function () {
              core.navigateTo(element, tools.getCellSize() * -7);
            }))
            .append($('<button type="button" class="nav-link nav-end"/>').html('&gt;&#124;').click(function () {
      ...
      // **Progress Bar** Return an element representing a progress of position within
      // the entire chart
      createProgressBar: function (label, desc, classNames, dataObj) {
        label = label || "";
        
        var bar = $('<div class="bar"><div class="fn-label">' + label + '</div></div>').data("dataObj", dataObj);
        if (desc) {
          bar
      ...
      // Remove the `wd` (weekday) class and add `today` class to the current
      // day/week/month (depending on the current scale)
      ...
      // **Fill the Chart** Parse the data and fill the data panel
      ...
      mouseScroll: function (element, e) {
        ...
        var mPos = element.scrollNavigation.mouseX === null
          ? e.pageX
      ...
      wheelScroll: function (element, e) {
        ...
        // simpler normalization, ignoring per-device/browser/platform acceleration &
        // semantic variations var delta = e.detail || - (e = e.originalEvent).wheelData
        // || e.deltaY /* || e.deltaX */ || e.detail; delta = ( delta / Math.abs(delta)
        // ) || 0;
    ...
    var tools = {
      ...
      getMaxDate: function (element) {
        ...
        switch (settings.scale) {
          ...
          case "weeks":
            ...
            maxDate = new Date(bd.getFullYear(), bd.getMonth(), md === 0
              ? 4
              : md - 3);
            break;
          case "months":
      ...
      getMinDate: function (element) {
        ...
        switch (settings.scale) {
          ...
          case "weeks":
            ...
            bd = new Date(bd.setDate(bd.getDate() - 3 * 7));
            var md = Math.floor(bd.getDate() / 7) * 7;
            minDate = new Date(bd.getFullYear(), bd.getMonth(), md === 0
              ? 4
              : md - 3);
            break;
          case "months":
      ...
      // Return an array of Date objects between `from` and `to`, scaled hourly
      ...
      // Return an array of Date objects between a range of weeks between `from` and
      // `to`
      ...
      // Return an array of Date objects between a range of months between `from` and
      // `to`
      ...
      // Returns true when the given date appears in the array of holidays, if
      // provided
      isHoliday: (function () { // IIFE
        ...
        var holidays = false;
        // returns the function that will be used to check for holidayness of a given
        // date
        return function (date) {
          if (!holidays) {
            holidays = tools._datesToDays(settings.holidays);
          }
          return !!holidays[// assumes numeric dates are already normalized to start-of-day
            $.isNumeric(date)
              ? date
      ...
      getCellSize: function () {
        if (typeof tools._getCellSize === "undefined") {
          var measure = $('<div style="display: none; position: absolute;" class="fn-gantt"><div class="row' +
              '"></div></div>');
          $("body").append(measure);
          tools._getCellSize = measure
            .find(".row")
            .height();
          measure
            .empty()
